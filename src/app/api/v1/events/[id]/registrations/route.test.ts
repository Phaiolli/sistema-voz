...
describe("GET /api/v1/events/[id]/registrations — event isolation (C2 BOLA)", () => {
  // isEventOwnedBy queries `events` with .select().eq().eq().maybeSingle()
  ...
  it("returns 404 when owner B requests registrations of owner A's event", async () => {
    vi.mocked(auth).mockResolvedValue({ user: { id: "owner_b", role: "owner" } } as never);
    mockSupabase.from.mockImplementation((table: string) =>
      table === "events" ? ownershipChain(false) : { select: vi.fn().mockReturnThis(), eq: vi.fn().mockReturnThis(), order: vi.fn().mockResolvedValue({ data: [], error: null }) },
    );
    const res = await GET(makeReq(), makeParams());
    expect(res.status).toBe(404);
  });

  it("returns the list when owner owns the event", async () => {
    vi.mocked(auth).mockResolvedValue({ user: { id: "owner_a", role: "owner" } } as never);
    mockSupabase.from.mockImplementation((table: string) =>
      table === "events"
        ? ownershipChain(true)
        : { select: vi.fn().mockReturnThis(), eq: vi.fn().mockReturnThis(), order: vi.fn().mockResolvedValue({ data: [mockReg], error: null }) },
    );
    const res = await GET(makeReq(), makeParams());
    expect(res.status).toBe(200);
  });
  ...
  it("returns 403 when a mediador is not assigned to the event", async () => {
    ...
    mockSupabase.from.mockImplementation((table: string) =>
      table === "mediator_assignments"
        ...
        : { select: vi.fn().mockReturnThis(), eq: vi.fn().mockReturnThis(), order: vi.fn().mockResolvedValue({ data: [], error: null }) },
    );
    const res = await GET(makeReq(), makeParams());
    expect(res.status).toBe(403);
  });
